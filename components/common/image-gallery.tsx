    ...
    <>
      <div className="space-y-3">
        ...
        <div className="relative aspect-video rounded-lg overflow-hidden bg-muted group">
          ...
          {/* Image Counter */}
          <div className="absolute bottom-3 right-3 bg-black/60 text-foreground text-sm px-3 py-1 rounded-full">
      ...
      {isLightboxOpen && (
        <div className="fixed inset-0 z-50 bg-black/95 flex items-center justify-center">
          <Button
            variant="ghost"
            size="icon"
            className="absolute top-4 right-4 text-foreground hover:bg-white/10"
          ...
          <Button
            variant="ghost"
            size="icon"
            className="absolute left-4 top-1/2 -translate-y-1/2 text-foreground hover:bg-white/10"
            onClick={handlePrevious}
          >
          ...
          <Button
            variant="ghost"
            size="icon"
            className="absolute right-4 top-1/2 -translate-y-1/2 text-foreground hover:bg-white/10"
            onClick={handleNext}
          >
            <ChevronRight className="h-8 w-8" />
          </Button>

          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-black/60 text-foreground text-sm px-4 py-2 rounded-full">
            {currentIndex + 1} / {images.length}
          </div>
        </div>
      )}
