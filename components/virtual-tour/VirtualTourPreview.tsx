...
export const VirtualTourPreview = ({
  ...
  return (
    <div className={containerClasses} onMouseMove={handleMouseMove} style={!isFullscreen ? { height } : undefined}>
      <div className={cn("transition-opacity duration-500", transitioning ? "opacity-0" : "opacity-100")}>
        <VirtualTourViewer
          scene={currentScene}
          mode="preview"
            ...
            <div className="flex items-center gap-2">
              {!fullscreen && (
                <Button
                  ...
                  variant="ghost"
                  size="icon"
                  className="bg-black/30 hover:bg-black/50 text-foreground"
                >
        ...
        {scenes.length > 1 && (
          <>
            <Button
              ...
              variant="ghost"
              size="icon"
              className="absolute left-4 top-1/2 -translate-y-1/2 bg-black/30 hover:bg-black/50 text-foreground disabled:opacity-30 pointer-events-auto"
            >
            ...
            <Button
              ...
              variant="ghost"
              size="icon"
              className="absolute right-4 top-1/2 -translate-y-1/2 bg-black/30 hover:bg-black/50 text-foreground disabled:opacity-30 pointer-events-auto"
            >
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/50 to-transparent pointer-events-auto">
          <div className="flex items-center justify-center">
            <div className="bg-black/70 backdrop-blur-sm rounded-full px-6 py-3 flex items-center gap-4">
              ...
              <span className="text-foreground text-sm font-medium">{currentScene?.sceneName}</span>
            ...
            {scenes.map((scene, idx) => (
              <button
                ...
                className={cn(
                  ...
                  idx === currentSceneIndex ? "w-8 bg-emerald-500" : "w-2 bg-white/40 hover:bg-white/60",
                )}
              />
            ))}
          </div>
      ...
      {transitioning && (
        <div className="absolute inset-0 bg-black/50 flex items-center justify-center pointer-events-none">
          <div className="text-foreground text-sm">Transitioning...</div>
        </div>
      )}
    </div>
  );
