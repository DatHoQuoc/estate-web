...
export const VirtualTourEditor = () => {
  ...
  const handleAddHotspot = async () => {
    ...
    try {
      ...
      setScenes((prev) =>
        prev.map((s, idx) => (idx === currentSceneIndex ? { ...s, hotspotsJson: updatedHotspots } : s)),
      );
        ...
        <div className="max-w-7xl mx-auto py-6 px-4">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div className="flex-1">
              <h1 className="text-2xl font-bold text-foreground">Virtual Tour Editor</h1>
              <p className="text-muted-foreground text-sm mt-1">
                Click on the 360° panorama to add hotspots at exact positions
              </p>
            </div>
            <Button onClick={() => navigate(`/seller/listings/${listingId}`)}>Save & Return</Button>
          </div>
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-medium truncate">{scene.sceneName}</p>
                        </div>
                      </div>
                    </button>
                  ))}
                  ...
                  <div className="space-y-3">
                    <div className="grid grid-cols-2 gap-3 text-xs">
                      <div>
                        ...
                        <Input value={currentScene?.sceneName || ""} className="h-8 text-xs" readOnly />
                      </div>
                      <div>
              ...
              <Card>
                ...
                <CardContent className="space-y-2 max-h-[200px] overflow-y-auto">
                  ...
                  {currentScene?.hotspotsJson.map((hotspot) => (
                    <div key={hotspot.id} className="flex items-start justify-between p-2 bg-muted rounded-lg">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          {hotspot.type === "info" ? (
          ...
          <Card className="mb-4">
            <CardHeader className="pb-3">
              <CardTitle className="text-sm">360° Panorama Editor - {currentScene?.sceneName || "No scene"}</CardTitle>
            </CardHeader>
            <CardContent>
              <VirtualTourViewer
                scene={currentScene}
                      ...
                      <div key={scene.sceneId} className="flex items-center gap-1">
                        <div
                          className={cn(
                            ...
                            idx === currentSceneIndex
                              ? "bg-emerald-500 text-foreground scale-110"
                              : "bg-muted text-muted-foreground",
                          )}
                        >
                          {idx + 1}
                        </div>
                        {idx < scenes.length - 1 && <div className="w-8 h-0.5 bg-muted" />}
                      </div>
                    ))}
                  </div>
                  <p className="text-xs text-muted-foreground">
                    Tour contains {scenes.length} scene{scenes.length !== 1 ? "s" : ""} with{" "}
                    {scenes.reduce((acc, s) => acc + s.hotspotsJson.length, 0)} total hotspots
                  </p>
                </div>
              </div>
            </CardContent>
