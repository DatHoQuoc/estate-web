  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-2.5">
        ...
        >
          {allPass ? "All checks passed" : `${failCount} issue${failCount > 1 ? "s" : ""} to fix`}
        </span>
        {allPass ? <Check className="h-4 w-4 text-emerald-600" /> : <AlertCircle className="h-4 w-4 text-amber-600" />}
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-1.5">
        {items.map((item) => (
          <div key={item.key} className="flex items-center gap-2">
            ...
            <span className={cn("text-xs", item.passed ? "text-emerald-800" : "text-amber-800 font-medium")}>
              {item.label}
            </span>
          </div>
        ))}
...
function MediaStrip({ data }: { data: ReviewData }) {
  ...
  const otherImages = data.images.filter((img) => img.id !== cover?.id).slice(0, 3);
    ...
    <div className="flex gap-2 items-stretch">
      ...
      {cover && (
        <div className="relative w-2/5 min-h-[140px] rounded-lg overflow-hidden flex-shrink-0 border border-muted">
          <img src={cover.preview || "/placeholder.svg"} alt="Cover" className="object-cover w-full h-full" />
      ...
      <div className="flex-1 flex flex-col gap-2 min-w-0">
        ...
        <div className="flex gap-2 flex-1">
          {otherImages.map((img) => (
            <div key={img.id} className="flex-1 rounded-lg overflow-hidden border border-muted">
              <img src={img.preview || "/placeholder.svg"} alt="Preview" className="object-cover w-full h-full" />
            </div>
          ))}
          {data.images.length > 4 && (
            <div className="flex-shrink-0 w-14 rounded-lg bg-muted border border-muted flex items-center justify-center">
              <span className="text-xs font-semibold text-muted-foreground">+{data.images.length - 4}</span>
            </div>
          )}
        </div>
        ...
        <div className="flex gap-2 flex-wrap">
          <span className="inline-flex items-center gap-1.5 text-xs bg-muted border border-muted rounded-full px-2.5 py-1">
            ...
            <span className="text-muted-foreground">{data.images.length} photos</span>
          </span>
          ...
          {has360 && (
            <span className="inline-flex items-center gap-1.5 text-xs bg-emerald-50 border border-emerald-200 rounded-full px-2.5 py-1">
              ...
              {pending360 > 0 && <span className="text-emerald-500 font-normal">· {pending360} pending</span>}
            </span>
          )}
          ...
          {hasVideos && (
            <span className="inline-flex items-center gap-1.5 text-xs bg-blue-50 border border-blue-200 rounded-full px-2.5 py-1">
              ...
              {pendingVideos > 0 && <span className="text-blue-500 font-normal">· {pendingVideos} pending</span>}
            </span>
          )}
        </div>
      </div>
    ...
    <div className="space-y-5">
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base font-semibold">Listing Preview</CardTitle>
        </CardHeader>
        ...
        {detailsOpen && (
          <CardContent className="pt-0 space-y-5">
            ...
            <div className="pt-4 border-t border-muted">
              ...
              <p className="text-sm text-foreground whitespace-pre-line leading-relaxed">{data.description}</p>
            </div>
                ...
                {data.direction && (
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold">{directionLabels[data.direction] || data.direction}</p>
                    </div>
                  </div>
                )}
                {data.legalStatus && (
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold">{legalStatusLabels[data.legalStatus] || data.legalStatus}</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                <div className="space-y-1.5">
                  {data.images360.map((scene, idx) => (
                    <div key={scene.id} className="flex items-center gap-3 px-3 py-2 bg-muted/40 rounded-lg">
            ...
            {data.features.length > 0 && (
              <div>
                <h4 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2.5">Features</h4>
                <div className="flex flex-wrap gap-2">
                  {data.features.map((f) => (
                    <span
                      key={f}
