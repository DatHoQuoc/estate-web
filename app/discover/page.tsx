...
export default function DiscoveryHomePage() {
  ...
  const featured = useMemo(() => listings.slice(0, 3), [listings])
  ...
  const goToAssistant = (q?: string) => {
    ...
    navigate(`/discover/assistant${params.toString() ? `?${params.toString()}` : ""}`)
  }
      ...
      <main className="pt-6">
        <section className="bg-muted/50 border-b border-border">
          <div className="max-w-6xl mx-auto px-4 py-12 space-y-8">
            <div className="flex flex-col gap-4">
              <p className="text-xs uppercase tracking-[0.2em] text-primary font-semibold">Discovery & Connection</p>
        ...
        <section className="max-w-6xl mx-auto px-4 py-12 space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-muted-foreground">Discovery Home</p>
          ...
          {loading && <p className="text-sm text-muted-foreground">Loading featured listings...</p>}
